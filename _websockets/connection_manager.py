...
from agents import copilot_agent_graph, web_agent_graph
...
class ConnectionManager:
    ...
    async def web_search(
        self, user_id , session_id: int, ws_message: MessageData, websocket: WebSocket):
        try:
    ...
    async def copilot_mode(
        self, user_id , session_id,ws_message: MessageData, websocket: WebSocket) -> None:
        try:
            # result = await copilot_agent_graph()
            processing_config = {
                "configurable": {
    ...
    async def handle_message(
        ...
        if data.focus_mode == "copilot":
            await self.copilot_mode(user_id, session_id, ws_message, websocket)
        elif data.focus_mode == "comparison":
            await self.comprison_mode(websocket)
        elif data.focus_mode == "all":
            await self.web_search(user_id, session_id, ws_message, websocket)
