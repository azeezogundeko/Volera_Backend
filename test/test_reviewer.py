...
from agents import State, agent_manager, reviewer_agent_node
...
request = WSMessage(
    ...
    files=["file1", "file2"],
    message={
        "content": "What is the cheapest gaming mouse?"
    },
    history=[],
...
state = State(
    ...
    agent_results={
        ...
        agent_manager.meta_agent: {
            ...
            "instructions": "Find the cheapest gaming mouse available on the market.  Consider all major retailers and brands."
        }
    },
    final_result={},
    chat_limit=10,
...
async def test_reviwer_agent_node():
    result = await reviewer_agent_node(state)


if __name__ == "__main__":
    import asyncio
    asyncio.run(test_reviwer_agent_node())
